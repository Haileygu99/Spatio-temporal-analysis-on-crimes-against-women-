---
title: "Spatio-temporal analysis of rape crimes against women in Uttar Pradesh (India)"
author: "Hailey Gu"
date: "2023-03-28"
...
1. Import the Rdata file with the data and call the data.frame object as *data*. 
```{r}
load("~/Desktop/AA project/DS1_CrimeUttarPradesh/CrimeUttarPradesh.RData")
```

2. Compute the total number of cases of rapes per year, and format the output in a table. To display a table with additional formatting, we can use the `knitr::kable` function:
...
6. Aggregate observed and expected rape cases over geographical areas `ID_area`.
```{r echo=TRUE, eval=TRUE}
...
7. compute the standardized morbidity ratios for the rape cases (SMRs)
```{r eval=TRUE, echo=TRUE}
...
```{r mapSMR, eval=TRUE, echo=TRUE, fig.cap="Map of the average rape SMRs over the period 2001-2014 in Uttar Pradesh"}
...
```{r mod_ST, eval=TRUE}
formula_ST_noint = rape_obs ~ f(ID.space, model="bym2", graph=carto_up_sf.adj,
                            hyper=list(prec = list(
                            prior = "pc.prec",
                            param = c(0.5 / 0.31, 0.01)),
                            phi = list(
...
```{r mapRRst, eval=TRUE, echo=TRUE, include=TRUE, fig.cap="Spatio-temporal model: Map of the residual RRs and posterior probabilities for rape cases", , fig.width=12, fig.height=10}
...
```{r mapRRstIntI, eval=TRUE, echo=TRUE, include=TRUE, fig.cap="Spatio-temporal model: Map of the residual RRs and posterior probabilities for rape cases", , fig.width=12, fig.height=10}
...
ANSWER:
We basically see that across the different models there is tiny difference in the spatial residuals. Let's now look at the space-time interaction.
